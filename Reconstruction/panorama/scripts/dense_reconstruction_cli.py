#!/usr/bin/env python3
"""
COLMAP稠密重建脚本（使用命令行工具）- 全景图版本
从稀疏重建结果生成稠密点云
参考: denseline.sh

注意: 全景图重建的图像目录结构为 images/pano_camera{N}/xxx.jpg
      深度图也会保存在子目录结构中，需要递归搜索
"""
...
def count_images_recursive(image_path: Path) -> int:
    """递归统计图像数量（全景图重建的图像在子目录中）"""
    count = 0
...
def run_dense_reconstruction_cli(
    ...
    if not skip_undistortion:
        ...
        if stereo_config.exists() and fusion_config.exists():
            ...
            if undistorted_images_path and undistorted_images_path.exists():
                # 统计输入图像和去畸变图像数量（递归搜索）
                input_image_count = count_images_recursive(image_path)
                undistorted_image_count = count_images_recursive(undistorted_images_path)
    ...
    if not skip_patch_match:
        ...
        if depth_maps_path.exists():
            # 统计深度图文件（递归搜索子目录，全景图的深度图保存在子目录中）
            ...
            # 获取去畸变图像数量（应该等于深度图数量）
            if undistorted_images_path and undistorted_images_path.exists():
                expected_depth_count = count_images_recursive(undistorted_images_path)
...
def main():
    parser = argparse.ArgumentParser(
        description="COLMAP稠密重建（使用命令行工具）- 全景图版本：从稀疏重建结果生成稠密点云",
    ...
    logging.info("=" * 60)
    logging.info("COLMAP 稠密重建（命令行工具）- 全景图版本")
    logging.info("=" * 60)
