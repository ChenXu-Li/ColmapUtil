...
def main():
    ...
    parser.add_argument("--stage_dir", type=str, default="/root/autodl-tmp/data/STAGE1_4x",
                       help="STAGE数据集根目录（用于加载原始点云）")
    parser.add_argument("--optimized_dir", type=str, default="/root/autodl-tmp/data/STAGE1_4x/BridgeB/elastic_refined",
                       help="优化后的点云目录（）")
    parser.add_argument("--load_original", action="store_false",
                       help="同时加载原始点云（在STAGE目录下的pointclouds文件夹）")
    parser.add_argument("--port", type=int, default=8081,
                       help="Viser服务器端口（默认8081，避免与原始脚本冲突）")
    ...
    parser.add_argument("--no_transform", action="store_true",
                       help="不对优化后的点云应用坐标变换（默认：应用变换，从camera坐标系转换到世界坐标系）")
